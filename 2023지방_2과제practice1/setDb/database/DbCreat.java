...
public class DbCreat {
	...
	public DbCreat() {
		...
		int result = DbManager.db.setData("-- -----------------------------------------------------\r\n"
				...
				+ "grant update, insert, delete, select on 2023지방_2.* to 'user'@'localhost';\r\n"
				+ "set session sql_mode = '';");
		int result1 = 0;
		Vector<Vector<String>> bookData = DbManager.
				db.getData("SELECT b_no FROM 2023지방_2.book;");
		for (Vector<String> val : bookData) {
			String num = val.get(0); 
			File file = new File("./datafiles/book/" + num + ".jpg");
			try {
				FileInputStream path = new FileInputStream(file);
				DbManager.db.setData("UPDATE `2023지방_2`.`book` SET `b_img` = ? WHERE (`b_no` = ?);\r\n"
						+ "", path, num);
